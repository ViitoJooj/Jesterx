...
function makeDefaultBundle(routePaths) {
  const routes = {};
...
function parseBundle(raw, routePaths) {
  ...
  try {
    ...
    if (typeof p.component === "string" && !p.routes) {
      const routes = {};
      (routePaths.length ? routePaths : ["/"]).forEach((path) => { routes[path] = { component: p.component }; });
...
const SHARED_FILES_DEFS = [
  { tab: "header",    label: "Header.svelte" },
  { tab: "footer",    label: "Footer.svelte" },
  { tab: "globalCss", label: "globals.css" },
];

export const SvelteEditor = () => {
  const { siteId = "" } = useParams();
  const { websiteId } = useAuthContext();
  const navigate = useNavigate();

  const [bundle, setBundle]       = useState({ header: DEFAULT_HEADER, footer: DEFAULT_FOOTER, globalCss: DEFAULT_CSS, routes: { "/": { component: DEFAULT_COMPONENT } } });
  const [routes, setRoutes]       = useState([]);
  const [active, setActive]       = useState({ kind: "route", path: "/" });
  ...
  function setRouteComponent(path, component) {
    setBundle((b) => ({ ...b, routes: { ...b.routes, [path]: { component } } }));
  }
  function setShared(field, val) {
  ...
  async function save(publish) {
    ...
    try {
      const resp = await apiFetch(`/api/v1/sites/${siteId}/versions`, {
  ...
  async function removeRoute(path) {
  ...
  const currentLang = active.kind === "shared" && active.tab === "globalCss" ? "css" : "svelte";
